...
//diafaneies mathimatos kai kwdikas apo  inet_str_server.cinet_dgr_server.c create_many_threads.c producer-consumer.c
int _pipeb = 0;
...
void initializeAggregator(char * serverIp, int serverPort, int w, int b, int total_countries, char **countries) {
    int i;
    _pipeb = b;
    ...
    // send serverIP IP and serverIP PORT to worker
    for (i = 0; i < w; i++) {
        writeall(down_fd[i], _serverIP, sizeof(_serverIP));
    }
    
    for (i = 0; i < w; i++) {
        writeall(down_fd[i], &serverPort, sizeof(serverPort));
    }
